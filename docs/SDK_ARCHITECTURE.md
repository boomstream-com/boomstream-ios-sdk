...
`BoomstreamPlayerController` (@MainActor): `play/pause`, `seek(to:)/seek(toPercent:)`, `setVolume/mute/unmute`, `next/previous` (плейлисты), `setFullScreen/toggleFullScreen`, `currentPosition/duration`, quality-API (`availableQualities`, `currentQuality`, `preferredQuality`, `setQuality(_:)`, `selectAuto()`, `qualityUpdates`) + стримы. В SwiftUI контроллер доступен через `BoomstreamPlayerProxy` после монтирования view; в UIKit — свойство `controller` у `BoomstreamPlayerUIView`.
...
Overlay поверх видео: play/pause, prev/next (видимы только для плейлистов), seek-слайдер с таймингами, кнопка fullscreen (иконка отражает состояние), опционально — кнопка выбора качества (`showQualitySelector`). Тап по видео — показать/скрыть; автоскрытие через 3,5 с при воспроизведении; на паузе контролы остаются. Отключаются `showsControls: false` / `isControlsEnabled = false`.

### Выбор качества

- **Модель** — публичный enum `VideoQuality`: `.auto` / `.resolution(height:peakBitRate:label:)`. Наружу — только примитивы (`Int`, `String`); AVFoundation-типы не экспонируются, что закреплено reflection-тестом (`PublicAPIReflectionTests`). Равенство/хэш — по высоте (два варианта одной высоты — один слот качества).
- **Обнаружение вариантов** — после `.readyToPlay` из master-манифеста: основной путь `AVAsset.load(.variants)` (`AVAssetVariant`), fallback — собственный парсер `EXT-X-STREAM-INF` (RESOLUTION/BANDWIDTH). Список сортируется по убыванию высоты и публикуется в `availableQualities` + `qualityUpdates` (`AsyncStream<[VideoQuality]>`).
- **Применение** — `setQuality(.resolution(height:))` ставит cap через `preferredMaximumResolution`/`preferredPeakBitRate` текущего `AVPlayerItem` — переключение живое, без перезагрузки и потери позиции; `.auto` снимает cap. Выбор переприменяется при смене трека плейлиста (новый `AVPlayerItem`). Без явного вызова `setQuality`/`selectAuto` initial-hint `AdvancedPlayerOptions.preferredPeakBitRate` сохраняется.
- **UI (opt-in)** — `AdvancedPlayerOptions.showQualitySelector: true` добавляет в контролы кнопку-шестерёнку: action sheet со списком `availableQualities` + Auto. По умолчанию выключено — раскладка контролов не меняется.
...
`AdvancedPlayerOptions` — whitelisted-параметры: `preferredForwardBufferDuration`, `automaticallyWaitsToMinimizeStalling`, `preferredPeakBitRate`, `showQualitySelector`.
